  ...
  where
    ...
    loop sum' _ = do
      ...
      let loss = func $ toDependent input
          (g:_) = grad loss [input]
      return $ Torch.abs g + sum'

main = do
  args <- getArgs
  when (length args /= 3) $ do
    putStrLn "Usage: darknet-cli config-file weight-file input-image-file"
  ...
  readImageAsRGB8WithScaling input_file 256 256 True >>= \case
    Right (input_image, input_tensor) -> do
      let input_data' = divScalar (255 :: Float) (hwc2chw $ toType Float input_tensor)
          (_,out) = forwardDarknet net' (Nothing, input_data')
          func idx img = (snd $ forwardDarknet net' (Nothing, img)) ! (0,idx,0,0)
      let idx = asValue $  argmax (Dim 0) RemoveDim $ squeezeAll out
      print $ idx
      print $ labels !! idx
      print $ (squeezeAll out) ! idx
      img <- smoothGrad 10 0.2 (\i -> func idx i) input_data'
      -- let v = sumAll input_data'
      -- img <- smoothGrad 10 0.2 (\i -> sumAll $ i * i) input_data'
      -- print img
      writePng "oo.png" $ toType UInt8 $ mulScalar (255.0::Float) $ clamp 0 1 $ chw2hwc $ input_data'
      writePng "o.png" $ toType UInt8 $ mulScalar (255.0::Float) $ clamp 0 1 $ chw2hwc $ 100 `mulScalar` img
    Left err -> print err
