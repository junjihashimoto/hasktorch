...
main = do
  let opt = \case
        [] -> ["traced_resnet_model.pt","elephant.jpg"]
        a@[model',input'] -> a
        _ -> error $ "Usage: smoothgrad model-file image-file"
...
-- model <- load WithoutRequiredGrad modelfile
  ...
  case mimg of
    ...
    Right (img_,_) -> do
      ...
      print $ shape img
      --writePng "o.png" $ toType UInt8 $ mulScalar (255.0::Float) $ deNormalize $ chw2hwc img''
      
      case (Torch.Script.forward model [IVTensor img'']) of
        IVTensor v'' -> do
          let v' = softmax (Dim 1) v''
          print $ shape v'
          print $ topK 5 (Dim 1) True True $ softmax (Dim 1) v'
          let (scores',idxs') = topK 5 (Dim 1) True True v'
